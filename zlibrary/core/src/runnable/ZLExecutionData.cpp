...
bool ZLExecutionData::terminateProcess(){
	if (!myListener.isNull()) {
			return myListener->terminateProcess();
		}
	else return false;
}
...
void ZLExecutionData::Listener::setTerminateFlag(){
	terminateFlag = true;
}
bool ZLExecutionData::Listener::terminateProcess(){
