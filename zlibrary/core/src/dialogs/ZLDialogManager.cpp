...
shared_ptr<ZLProgressDialog> ZLDialogManager::waitDialog;

void ZLDialogManager::wait(const ZLResourceKey &key, ZLRunnable &runnable) const {
	DBG("createProgressDialog");
	DBG("createProgressDialog run %s",key.Name.c_str());
//TODO waitDialog

	//shared_ptr<ZLProgressDialog> dialog = createProgressDialog(key);
	//if (!dialog.isNull()) {	dialog->run(runnable);	}
	waitDialog = createProgressDialog(key);
	if (!waitDialog.isNull()) {	waitDialog->run(runnable);	}
	else {	DBG("else runnable.run()");
		    runnable.run();
	}
}
