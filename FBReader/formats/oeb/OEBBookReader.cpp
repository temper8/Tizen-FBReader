...
void OEBBookReader::startElementHandler(const char *tag, const char **xmlattributes) {
	DBG("OEBBookReader::startElementHandler");
...
bool OEBBookReader::readBook(const ZLFile &file) {
	...
	DBG("OEBBookReader::readBook");
	myModelReader.setMainTextModel();
	myModelReader.pushKind(REGULAR);
    int n=0;
    DBG("myFilePrefix %s", myFilePrefix.c_str());
    std::string firstFile;
    firstFile = myFilePrefix + myHtmlFileNames[0];
    DBG("firstFile %s", firstFile.c_str());
...
void OEBBookReader::namespaceListChangedHandler() {
	...
	for (; iter != namespaceMap.end(); ++iter) {
		DBG("namespaceListChangedHandler ");
