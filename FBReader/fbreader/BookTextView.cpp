...
void BookTextView::gotoParagraph(int num, bool end) {
	DBG("BookTextView::gotoParagraph %d",num);
	if (textArea().isEmpty()) {
		return;
	}
...
bool BookTextView::onStylusClick(int x, int y, int count) {
	DBG("BookTextView::onStylusClick count=%d",count);
	...
	if (rectangle != 0) {
		...
		if (getHyperlinkInfo(*rectangle, id, type)) {
			DBG("tryShowFootnoteView %s, %s",id.c_str(),type.c_str());
