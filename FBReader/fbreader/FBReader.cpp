...
*/
	...
	if (!myBookAlreadyOpen) {
		DBG("FBReader::initWindow()   !myBookAlreadyOpen");
		shared_ptr<Book> book;
		if (!myBookToOpen.empty()) {
			DBG("FBReader::initWindow()   !myBookToOpen.empty()");
		...
		if (book.isNull()) {
			DBG("book.isNull() почитаем хелп на англицком");
...
void FBReader::openBookInternal(shared_ptr<Book> book) {
	if (!book.isNull()) {
		...
		DBG("bookTextView.setCaption %s", book->title().c_str());
		DBG("ZLTextHyphenator::Instance().load l=%s",book->language().c_str());
...
void FBReader::openLinkInBrowser(const std::string &url) const {
	...
	DBG("openLinkInBrowser url=%s",url.c_str());
/*
   int len = url.length();
   String bada_str;
   ByteBuffer buffer;
